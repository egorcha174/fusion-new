...
const NumberInput: React.FC<{ value: number | undefined, onChange: (val?: number) => void, min?: number, max?: number, placeholder?: string }> = ({ value, onChange, min, max, placeholder }) => (
    <input
        type="number"
        min={min}
        max={max}
          ...
          <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
            <main className="flex-1 flex flex-col items-center justify-center bg-gray-900/50 relative" onClick={() => handleSelect('element', '')}>
              <div ref={previewRef} className="w-[400px] transition-all duration-300 relative" style={{ aspectRatio: `${editedTemplate.width || 1} / ${editedTemplate.height || 1}`}}>
                  <div className="absolute inset-0 bg-center" style={{ backgroundImage: `linear-gradient(rgba(100,116,139,0.2) 1px, transparent 1px), linear-gradient(90deg, rgba(100,116,139,0.2) 1px, transparent 1px)`, backgroundSize: `${SNAP_GRID_SIZE * 10}px ${SNAP_GRID_SIZE * 10}px`}} />
            ...
            <div className="flex-grow p-4 space-y-4 overflow-y-auto no-scrollbar">
              { !selectedElement && !selectedSlot && (<>
                ...
                <Section title="Размеры карточки">
                    <LabeledInput label="Ширина" suffix="колонок"><NumberInput value={editedTemplate.width} onChange={v => setEditedTemplate(p => ({...p, width: v}))} min={1} max={10}/></LabeledInput>
                    <LabeledInput label="Высота" suffix="рядов"><NumberInput value={editedTemplate.height} onChange={v => setEditedTemplate(p => ({...p, height: v}))} min={1} max={10}/></LabeledInput>
                </Section>
