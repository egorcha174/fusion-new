...
import { Device, Tab, EventTimerWidget, CustomCardWidget } from '../types';
...
const HelpersPage: React.FC = () => {
    ...
    const handleEditCustomCard = (widget: CustomCardWidget) => {
                        ...
                        {[...eventTimerWidgets, ...customCardWidgets].map(widget => {
                            ...
                            return (
                                 <div key={widget.id} className="bg-white/80 dark:bg-gray-800/80 p-3 rounded-lg flex items-center justify-between ring-1 ring-black/5 dark:ring-white/5">
                                    <div className="flex items-center gap-4 overflow-hidden">
                                        <div className="w-8 h-8 flex-shrink-0 text-gray-500 dark:text-gray-400">
                                            <DeviceIcon icon={device.type} isOn={false} className="!w-full !h-full !m-0" />
                                        </div>
                                        <div className="flex-1 overflow-hidden">
                                            <p className="font-medium text-gray-900 dark:text-gray-100 text-sm truncate">{widget.name}</p>
                                            <p className="text-xs text-gray-500 dark:text-gray-400">{isTimer ? 'Виджет-таймер' : 'Кастомная карточка'}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2 flex-shrink-0">
                                        <button onClick={() => isTimer ? setEditingEventTimerId(widget.id) : handleEditCustomCard(widget)} className="p-2 text-gray-500 dark:text-gray-400 hover:text-blue-500" title="Настроить">
