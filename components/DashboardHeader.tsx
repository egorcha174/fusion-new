...
const DashboardHeader: React.FC<DashboardHeaderProps> = ({
    ...
    const renderMenuItems = () => (
        <div className="py-1">
            ...
            <button onClick={() => { onNavigate('icon-converter'); setIsMenuOpen(false); setIsMobileMenuOpen(false); }} className="flex items-center gap-3 w-full text-left px-4 py-2 text-sm text-gray-200 hover:bg-gray-700">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                   <path strokeLinecap="round" strokeLinejoin="round" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
                </svg>
                <span>Конвертер иконок</span>
            </button>
        </div>
    );
