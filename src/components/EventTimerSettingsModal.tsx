...
const EventTimerSettingsModal: React.FC<EventTimerSettingsModalProps> = ({ widgetId, onClose, currentColorScheme }) => {
    ...
    const handleNumberInput = (value: string, setter: (val: number) => void, allowUndefined: boolean = false) => {
                ...
                <div className="flex">
                    <div className="w-1/2 p-6 space-y-4 max-h-[70vh] overflow-y-auto no-scrollbar border-r border-gray-200 dark:border-gray-700">
                        ...
                        <LabeledInput label="Интервал (дней)">
                            <input type="number" min="1" value={cycleDays} onChange={e => handleNumberInput(e.target.value, setCycleDays)} className="w-full bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </LabeledInput>
                        ...
                        <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                            ...
                            <div className="space-y-2 bg-gray-100 dark:bg-gray-700/50 p-3 rounded-lg">
                                ...
                                <LabeledInput label="Размер шрифта (px)">
                                    <input type="number" placeholder="Авто (18px)" value={nameFontSize ?? ''} onChange={e => handleNumberInput(e.target.value, setNameFontSize, true)} className="w-full bg-white dark:bg-gray-800/50 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
                                </LabeledInput>
                                <div className="grid grid-cols-2 gap-2">
                                    <LabeledInput label="Позиция X (%)">
                                        <input type="number" min="0" max="100" value={namePosition.x} onChange={e => handleNumberInput(e.target.value, (val) => setNamePosition(p => ({ ...p, x: val })))} className="w-full bg-white dark:bg-gray-800/50 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
                                    </LabeledInput>
                                    <LabeledInput label="Позиция Y (%)">
                                        <input type="number" min="0" max="100" value={namePosition.y} onChange={e => handleNumberInput(e.target.value, (val) => setNamePosition(p => ({ ...p, y: val })))} className="w-full bg-white dark:bg-gray-800/50 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
                                    </LabeledInput>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2 bg-gray-100 dark:bg-gray-700/50 p-3 rounded-lg mt-3">
                                ...
                                <LabeledInput label="Размер шрифта (px)">
                                    <input type="number" placeholder="Авто (88px)" value={daysRemainingFontSize ?? ''} onChange={e => handleNumberInput(e.target.value, setDaysRemainingFontSize, true)} className="w-full bg-white dark:bg-gray-800/50 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
                                </LabeledInput>
                                <div className="grid grid-cols-2 gap-2">
                                    <LabeledInput label="Позиция X (%)">
                                        <input type="number" min="0" max="100" value={daysRemainingPosition.x} onChange={e => handleNumberInput(e.target.value, (val) => setDaysRemainingPosition(p => ({ ...p, x: val })))} className="w-full bg-white dark:bg-gray-800/50 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
                                    </LabeledInput>
                                    <LabeledInput label="Позиция Y (%)">
                                        <input type="number" min="0" max="100" value={daysRemainingPosition.y} onChange={e => handleNumberInput(e.target.value, (val) => setDaysRemainingPosition(p => ({ ...p, y: val })))} className="w-full bg-white dark:bg-gray-800/50 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
                                    </LabeledInput>
                                </div>
                            </div>
                        </div>
