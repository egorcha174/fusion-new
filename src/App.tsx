...
const App: React.FC = () => {
    ...
    // Auto-close settings when connected successfully
    useEffect(() => {
        if (connectionStatus === 'connected') {
            setSettingsOpen(false);
        }
    }, [connectionStatus, setSettingsOpen]);
  ...
  const filteredRoomsForPhysicalDevicesPage = useMemo(() => {
    ...
    const backgroundStyle = useMemo((): React.CSSProperties => {
        ...
        if (scheme.dashboardBackgroundType === 'gradient') {
            return { backgroundImage: `linear-gradient(160deg, var(--bg-dashboard-1), var(--bg-dashboard-2))` };
            ...
            return { backgroundColor: 'var(--bg-dashboard-1)' };
        }
    }, [currentColorScheme]);
    ...
    // ... Context Menu Logic ... 
    const cardSizes = [ { w: 1, h: 0.5 }, { w: 1, h: 1 }, { w: 1, h: 2 }, { w: 2, h: 0.5 }, { w: 2, h: 1 }, { w: 2, h: 2 }, { w: 3, h: 1 }, { w: 3, h: 2 }, ];
    const handleDeviceContextMenu = useCallback((deviceId: string, tabId: string, x: number, y: number) => { setContextMenu({ x, y, deviceId, tabId }); }, [setContextMenu]);
    const handleGlobalContextMenu = useCallback((event: React.MouseEvent) => {
        ...
        if (deviceTarget && typeof deviceId === 'string' && typeof tabId === 'string') {
            ...
            if (activeTabId) setContextMenu({ x: event.clientX, y: event.clientY, deviceId: 'dashboard-global', tabId: activeTabId });
        }
  ...
  const isTemplateable = contextMenuDevice ? [ DeviceType.Sensor, DeviceType.DimmableLight, DeviceType.Light, DeviceType.Switch, DeviceType.Thermostat, DeviceType.Humidifier, DeviceType.Custom ].includes(contextMenuDevice.type) : false;
  ...
  const renderPage = () => {
    switch (currentPage) {
      case 'helpers': return <Suspense fallback={<div />}><HelpersPage /></Suspense>;
      case 'all-devices': return <AllDevicesPage rooms={filteredRoomsForPhysicalDevicesPage} />;
      case 'template-gallery': return <Suspense fallback={<div />}><TemplateGallery /></Suspense>;
      case 'dashboard':
      default:
        return activeTab ? (
          <ErrorBoundary>
            <TabContent key={activeTab.id} tab={activeTab} isEditMode={isEditMode} currentColorScheme={currentColorScheme} isDark={isDark} />
          </ErrorBoundary>
        ) : (
          <div className="text-center text-gray-500 mt-20">Выберите или создайте вкладку</div>
        );
    }
  };
    ...
    <>
      ...
      <div className="flex min-h-screen relative flex-col lg:flex-row" onContextMenu={handleGlobalContextMenu}>
        ...
        <Suspense fallback={<div className="bg-gray-900 hidden lg:block" style={{ width: `${sidebarWidth}px` }} />}>
          <InfoPanel sidebarWidth={sidebarWidth} setSidebarWidth={setSidebarWidth} haUrl={haUrl} signPath={signPath} getConfig={getConfig} colorScheme={currentColorScheme} isDark={isDark} />
        </Suspense>
        )}
        <div className="flex flex-col flex-1 h-screen overflow-hidden" style={{ marginLeft: isLg && isSidebarVisible ? `${sidebarWidth}px` : '0px' }}>
          <Suspense fallback={<div className="h-[73px] bg-gray-900 border-b border-gray-700/50" />}>
              <DashboardHeader currentColorScheme={currentColorScheme} isDark={isDark} />
          </Suspense>
      ...
      {confirmingDeleteWidget && (
        <Suspense fallback={null}>
          <ConfirmDialog isOpen={!!confirmingDeleteWidget} title="Удалить виджет?" message={<>Вы уверены, что хотите удалить виджет <strong className="text-black dark:text-white">"{confirmingDeleteWidget.name}"</strong>? Это действие нельзя отменить.</>} onConfirm={() => { deleteCustomWidget(confirmingDeleteWidget.id); setConfirmingDeleteWidget(null); }} onCancel={() => setConfirmingDeleteWidget(null)} />
        </Suspense>
      )}
      ...
      {contextMenu && (
        <Suspense fallback={null}>
          <ContextMenu x={contextMenu.x} y={contextMenu.y} isOpen={!!contextMenu} onClose={handleCloseContextMenu}>
            {isGlobalMenu ? (
                <>
                    <div onClick={() => { setIsEditMode(!isEditMode); handleCloseContextMenu(); }} className="px-3 py-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700/80 cursor-pointer text-sm flex items-center gap-2"><Icon icon={isEditMode ? "mdi:check" : "mdi:pencil"} className="w-4 h-4" />{isEditMode ? 'Завершить редактирование' : 'Режим редактирования'}</div>
                    ...
                    <div onClick={() => { setSettingsOpen(true); handleCloseContextMenu(); }} className="px-3 py-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700/80 cursor-pointer text-sm">Настройки</div>
                    <div onClick={() => { window.location.reload(); }} className="px-3 py-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700/80 cursor-pointer text-sm text-gray-500">Обновить страницу</div>
                </>
            ) : contextMenuDevice ? (
              <>
                ...
                {isTemplateable && (
                  <div onClick={() => { const templateToEdit = getTemplateForDevice(contextMenuDevice) || createNewBlankTemplate(contextMenuDevice.type); setEditingTemplate(templateToEdit); handleCloseContextMenu(); }} className="px-3 py-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700/80 cursor-pointer text-sm">Редактор шаблона</div>
                )}
                <SubMenuItem title="Размер">
                    {cardSizes.map(size => ( <div key={`${size.w}x${size.h}`} onClick={() => { useAppStore.getState().handleDeviceResizeOnTab(contextMenu.tabId, contextMenu.deviceId, size.w, size.h); handleCloseContextMenu(); }} className="px-3 py-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700/80 cursor-pointer text-sm" > {`${size.w} × ${String(size.h).replace('.', ',')}`} </div> ))}
                </SubMenuItem>
                <div className="h-px bg-gray-300 dark:bg-gray-600 my-1 mx-1" />
                {otherTabs.length > 0 && (
                    <SubMenuItem title="Переместить"> {otherTabs.map(tab => ( <div key={tab.id} onClick={() => { useAppStore.getState().handleDeviceMoveToTab(contextMenuDevice, contextMenu.tabId, tab.id); handleCloseContextMenu(); }} className="px-3 py-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700/80 cursor-pointer text-sm"> {tab.name} </div> ))} </SubMenuItem>
                )}
                {otherTabs.length > 0 && (
                    <SubMenuItem title="Копировать"> {otherTabs.map(tab => ( <div key={tab.id} onClick={() => { useAppStore.getState().handleDeviceCopyToTab(contextMenuDevice, tab.id); handleCloseContextMenu(); }} className="px-3 py-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700/80 cursor-pointer text-sm"> {tab.name} </div> ))} </SubMenuItem>
                )}
            ...
            {!isGlobalMenu && contextMenuDevice?.type === DeviceType.EventTimer && contextMenuDevice.widgetId && (
              <>
                ...
                <div onClick={() => { const widget = eventTimerWidgets.find(w => w.id === contextMenuDevice.widgetId); if (widget) setConfirmingDeleteWidget(widget); handleCloseContextMenu(); }} className="px-3 py-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700/80 cursor-pointer text-sm text-red-600 dark:text-red-400">Удалить виджет</div>
                 <div className="h-px bg-gray-300 dark:bg-gray-600 my-1 mx-1" />
