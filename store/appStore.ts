...
import {
  ...
  // FIX: Import 'CardElement' to resolve TypeScript error.
  CardElement
} from '../types';
...
export const useAppStore = create<AppState & AppActions>((set, get) => ({
    ...
    handleDeviceAddToTab: (device, tabId) => {
        const { getTemplateForDevice, checkCollision, setTabs } = get();
        const newTabs = get().tabs.map(tab => {
            ...
            // Iterate through half-rows to find a spot
            for (let r = 0; r <= rows - templateHeight; r += 0.5) {
                // Full-height items can only be placed on full rows
                ...
                for (let c = 0; c <= cols - requiredWidth; c++) {
                    ...
                    if (!checkCollision(tab.layout, itemToPlace, tab.gridSettings, '')) { // No device to ignore on add
                        placeAt = { col: c, row: r };
                        break;
                    }
                }
            ...
            return tab; // No space found, return original tab
        });
        setTabs(newTabs);
    },
    ...
    handleAddPhysicalDeviceAsCustomCard: (physicalDevice, tabId) => {
        ...
        if (!template) {
            ...
            const colWidth = 45;
            const startX = 27.5; // (100 - (2 * 45 + 5)) / 2 + 45/2 = 2.5 + 22.5
            const colGap = 50;
    ...
    createNewBlankTemplate: (deviceType: DeviceType | 'custom') => {
        const createElementsWithDefaults = (elements: CardElement[]): CardElement[] => {
            return elements.map(el => ({
                ...
                uniqueId: nanoid(), // Ensure uniqueId is fresh for copies
            }));
        };
        ...
        // Ensure all elements in newly created templates have default properties
