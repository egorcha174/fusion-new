...
const DEFAULT_COLOR_SCHEME: ColorScheme = {
  light: {
    dashboardBackground: '#e5e7eb', nameTextColor: '#1f2937', statusTextColor: '#6b7280', valueTextColor: '#111827', unitTextColor: '#6b7280',
    sidebarBackground: 'rgba(255, 255, 255, 0.7)', cardBackground: 'rgba(255, 255, 255, 0.8)', cardBackgroundOn: '#f3f4f6', headerBackground: 'rgba(255, 255, 255, 0.7)',
    tabTextColor: '#6b7280', activeTabTextColor: '#111827', tabIndicatorColor: '#111827', thermostatHandleColor: '#1f2937', thermostatDialTextColor: '#111827',
    thermostatDialLabelColor: '#6b7280', thermostatHeatingColor: '#f97316', thermostatCoolingColor: '#3b82f6', clockTextColor: '#111827',
    nameTextColorOn: '#111827', statusTextColorOn: '#4b5563', valueTextColorOn: '#111827', unitTextColorOn: '#4b5563',
  },
  dark: {
    dashboardBackground: '#111827', nameTextColor: '#d1d5db', statusTextColor: '#9ca3af', valueTextColor: '#f9fafb', unitTextColor: '#9ca3af',
...
interface AppActions {
    ...
    checkCollision: (layout: GridLayoutItem[], itemToPlace: { col: number; row: number; width: number; height: number; }, gridSettings: { cols: number; rows: number; }, ignoreDeviceId: string, originalItem?: GridLayoutItem) => boolean;
...
export const useAppStore = create<AppState & AppActions>((set, get) => ({
    ...
    checkCollision: (layout, itemToPlace, gridSettings, ignoreDeviceId, originalItem) => {
        const { col, row, width, height } = itemToPlace;
        if (col < 0 || row < 0 || col + Math.ceil(width) > gridSettings.cols || row + Math.ceil(height) > gridSettings.rows) return true;
        for (const existingItem of layout) {
            ...
            if (col < existingItem.col + existingWidth && col + width > existingItem.col && row < existingItem.row + existingHeight && row + height > existingItem.row) return true;
        }
        return false;
    },
    ...
    handleDeviceResizeOnTab: (tabId, deviceId, newWidth, newHeight) => {
        const newTabs = get().tabs.map(tab => {
            ...
            if (get().checkCollision(tab.layout, newItemLayout, tab.gridSettings, deviceId, deviceItem)) return tab;
